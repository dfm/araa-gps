...
\begin{figure}[ht]
  ...
  \includegraphics[width=\linewidth]{figures/kernel-ops1.pdf}
  \includegraphics[width=\linewidth]{figures/kernel-ops2.pdf}
  \includegraphics[width=\linewidth]{figures/kernel-ops3.pdf}
...
In other words, instead of just fitting for the parameters of the mean model $\meanparams$ (see SECTION ABOVE), we can simultaneously fit for both $\meanparams$ and the hyperparameters $\hyperparams$.
The likelihood function defined in SOMEEQUATION is a function of both $\meanparams$ and $\hyperparams$, and we can use that function as an objective for a non-linear optimization routine to find the maximum likelihood parameter values, or in a Markov chain Monte Carlo (MCMC) procedure to marginalize over nuisance parameters.
