...
Since 2010, the popularity of GPs has grown significantly, and in 2021, more than 500 refereed papers referencing GPs were published in astrophysics literature.
...
The first example is a re-implmentation of one of the earliest uses of GPR in time-domain astronomy \citep{prh92a, prh92b}, using modern language and techniques.
...
For this example, we use simulated data because it is useful to know the ``ground truth'' to truly highlight the importance of taking nuisances into account.

\subsubsection{Example 1: The time delay of a gravitational lensed quasar}
\label{sec:quasar}

\dan{Update text. I have switched to fitting the real data and recover a result consistent with that from prh.}
...
\end{equation}
where $m_i \equiv m(x_i,\meanparams)$, with respect to $\meanparams$. Where does this come from?
...
\dan{I think we should explicitly re-write this as $\mathcal{L}(\meanparams,\,\hyperparams)$ and indicate that $K$ and $m$ depend on these parameters. We should really highlight that this is a drop-in replacement for the likelihood defined in the previous section.}
...
Unlike how they are typical presented in the machine learning literature, GP models in astrophysics will often---but not always---include non-trivial mean functions.
For example, in the example from \autoref{sec:sim_transit}, the mean function $m(t;\,\meanparams)$ is a physical transit model that is a function of the orbital parameters of the system, and includes a realistic limb-darkening model \citep{2002ApJ...580L.171M}.
...
Another example that is commonly used in the astrophysics literature \dan{ADD SOME CITATIONS} is the following ``quasi-periodic'' kernel
\begin{equation}
...
\subsubsection{Multi-dimensional inputs}
...
\begin{equation}
  \meanparams_\star,\hyperparams_\star = \argmax_{\meanparams,\hyperparams} \,\mathcal{L}(\meanparams,\hyperparams) = \argmax_{\meanparams,\hyperparams} \,\log p(\boldsymbol{y}\,|\,\meanparams,\hyperparams) \quad.
\end{equation}
...
\begin{equation}
  \meanparams,\hyperparams \sim p(\meanparams,\hyperparams\,|\,\boldsymbol{y})
\end{equation}
...
\autoref{sec:quasar} and \autoref{sec:sim_transit}, and specifically \autoref{fig:quasar} and \autoref{fig:sim_transit}, show the results of worked examples of an MCMC-based GPR workflow, without and with (respectively) a non-trival mean function.
